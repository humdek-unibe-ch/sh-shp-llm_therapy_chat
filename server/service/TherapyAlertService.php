...
class TherapyAlertService extends TherapyChatService
{
    ...
    /**
     * Create a danger detection alert based on LLM safety assessment.
     *
     * Called when the LLM's structured response indicates critical or
     * emergency danger level. No keyword matching — purely context-based.
     *
     * @param int $conversationId therapyConversationMeta.id
     * @param array $detectedConcerns Safety concern categories from LLM (e.g. ['suicide', 'self_harm'])
     * @param string $safetyMessage The LLM's safety message or the AI response content
     * @param string $extraNotificationEmails Comma-separated emails
     * @return int|bool
     */
    public function createDangerAlert($conversationId, $detectedConcerns, $safetyMessage, $extraNotificationEmails = '')
    {
        ...
        $concerns = implode(', ', $detectedConcerns);

        $message = "Safety concerns detected by AI: $concerns\nAI assessment: \"$safetyMessage\"";

        $metadata = array(
            'detected_concerns' => $detectedConcerns,
            'safety_message' => mb_substr($safetyMessage, 0, 500)
        );
    ...
    private function sendUrgentNotification($alertId, $llmConversationId, $alertType, $message, $extraNotificationEmails = '')
    {
        try {
            ...
            $body = "<h2>Urgent Therapy Alert</h2>"
                ...
                . "<p style='color:#666; font-size:0.9em;'>This alert was generated by the AI safety monitoring system. "
                . "The LLM assessed the conversation context and identified safety concerns.</p>";
